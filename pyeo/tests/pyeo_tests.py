...
def test_clip_raster(managed_noncontiguous_geotiff_dir):
    ...
    result_path = os.path.join(test_dir.path, "test_out.tif")
    pyeo.raster_manipulation.clip_raster(test_raster, test_aoi, result_path)
    result = gdal.Open(result_path)
...
def test_classify_change(managed_ml_geotiff_dir):
    ...
    model = None
    pyeo.classification.classify_image(test_path, model, test_dir.path, test_dir.path)
...
def test_reshape_raster_for_ml(managed_ml_geotiff_dir):
    ...
    array = pyeo.classification.reshape_raster_for_ml(array)
...
def test_reshape_ml_out_to_raster():
    ...
    out = pyeo.classification.reshape_ml_out_to_raster(ml_array, 10, 6)
...
def test_get_training_data(managed_ml_geotiff_dir):
    ...
    out = pyeo.classification.get_training_data(training_image, training_shapes)
